...
if (loggedIn.value) {
  await navigateTo('/authenticated-test')
}
</script>

<template>
  <main class="min-h-screen flex items-center justify-center bg-surface-50 dark:bg-surface-950 px-4">
    <Card class="w-full max-w-sm">
      <template #header>
        <div class="flex flex-col gap-2 text-center pt-8 px-8">
          <span class="text-xs font-medium tracking-[0.22em] uppercase text-surface-500">WTD</span>
        </div>
      </template>
      <template #title>
      ...
      <template #subtitle>
        <div class="text-center">Sign in to continue</div>
      </template>
      <template #content>
        <Button
          as="a"
          ...
          severity="secondary"
          outlined
          class="w-full !justify-center gap-3 mt-4"
        >
